...
## サポートする Hyprland バージョン

**Hyprland 0.55** で開発・動作確認しています。`hyprland-toplevel-export-v1`
プロトコルと JSON IPC(`hyprctl -j`)を備えた最近のバージョンであれば動作する
はずです。ワークスペース切り替えは classic なディスパッチャ構文と Lua IPC
レイヤ(`hl.dsp.*`)の両方に対応しています。
